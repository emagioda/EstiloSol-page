        ...
        <section className="flex w-full flex-col md:h-[calc(100vh-80px)] md:grid md:grid-cols-2 md:gap-0">
          {heroSections.map((section, index) => {
            ...
            return (
              <article
                key={section.title}
                className="relative flex min-h-[50vh] w-full flex-col items-center justify-center gap-4 px-4 py-8 text-center text-[var(--brand-cream)] sm:py-10 md:h-full md:min-h-0 md:py-0 md:px-8 md:text-left"
              >
                <div className="glass-panel flex h-full w-full flex-col rounded-2xl px-5 py-6 text-center transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl sm:rounded-3xl sm:px-6 sm:py-8 md:text-left">
                  <TitleTag
                  ...
                  <div className="mt-6 flex flex-1 flex-col items-center justify-between gap-4 sm:mt-8 sm:gap-6">
                    <div className="relative w-full overflow-hidden rounded-xl border border-[var(--brand-gold-300)]/60 bg-[rgba(255,255,255,0.1)] p-1 shadow-[0_0_35px_rgba(0,0,0,0.35)] sm:rounded-2xl sm:p-2">
                      <div className="relative flex h-32 w-full items-center justify-center overflow-hidden rounded-lg bg-[rgba(255,255,255,0.08)] sm:h-40 md:h-56 md:rounded-xl">
                        {hasImage ? (
                          <Image
