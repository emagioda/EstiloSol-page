...
vi.mock("@/src/server/payments/reconciliation", () => ({ reconcileMercadoPagoPayment: vi.fn() }));
...
describe("PR 2 webhook persistence boundary", () => {
  it("PR2-WEBHOOK-06 does not mark dedupe when general order persistence fails", async () => {
    vi.mocked(reconcileMercadoPagoPayment).mockRejectedValue(new Error("KV unavailable"));
